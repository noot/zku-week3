...
const encrypt = async (
  ...
  let iv = buf2Bigint(mimc7.multiHash(plaintext, BigInt(0)));

  const ciphertext: Ciphertext = {
      iv,
      data: plaintext.map((e: bigint, i: number): bigint => {
          return e + buf2Bigint(mimc7.multiHash(
              sharedKey,
              iv + BigInt(i),
          ))
      }),
  ...
  const mimc7 = await buildMimc7();
    ...
    const plaintext: Plaintext = ciphertext.data.map(
        (e: bigint, i: number): bigint => {
            return e - buf2Bigint(mimc7.multiHash(sharedKey, ciphertext.iv + BigInt(i)))
        }
    )
